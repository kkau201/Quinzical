...
;;;
;;;
;;; Generated by JT, adapted by CIW (23/3/09) to improve duration model.

;; not using the one below for the moment - too uncomplete.
...
;; Used the gsw_durs for rab voice ( same as that used for kal)
;; Used gsw_durs - as symbols for phones same as what we use - have had to make some modification - see below
...
(oo   0.138 0.046)
(uu   0.107 0.044)
(@@r   0.147 0.035) ;;value from MRPA @@
(ow   0.171 0.046)  ;; value from MRPA au
(ou   0.134 0.039)
(ei   0.130 0.042)
(ai   0.137 0.047)
(oi   0.183 0.050)
(ir   0.141 0.061) ;; avlue from MRPA i@
(@    0.046 0.020)
;; extra phones - which NZE needs according to JT's application of S. Fitts lexicon
;; CIW takes a punt at duration values.
(uw  0.107 0.044) ;; value from MRPA uu - need to check its need for existance
(t^ 0.048 0.021) ;; value from MRPA d
(m! 0.102 0.037);; value from MRPA s
(n! 0.102 0.037);; ditto
(l! 0.102 0.037);; ditto
(oul  0.171 0.046);; value from MRPA au
))

;;note this is the gsw duration cart tree (and the kal one) from the main festival release
;;I couldn't link directly to it - so just for the moment will do it this way.
